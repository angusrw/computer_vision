...
//Portion of sample you want to randomly select
int sample_random = 20;
//Value at which to threshold the Hough Space
int threshcircle = 50;
//Number of pixels containing concentric circles for the detector to trigger
int maxcircles = 6;
  ...
  int*** circle_space = malloc3dArray(x_dim, y_dim, r_dim);
	...
	for (int y = 0; y < frame_height; y++){
      ...
      if( intensity > 250) {
        ...
        int random = rand() % 100;
        //If the number is within the random sample range
        if(random < sample_random) {
