...
int main( int argc, const char** argv )
{
	...
	char coords[11][4];
  std::ifstream file("csvs/dart8board.csv");
...
void detectAndDisplay( Mat frame )
{
  ...
  for(int i = 0; i < trueboards; i++)
    ...
    //std::cout << coordinates[i][i] << std::endl;
		...
		{
		   int tl[2] = {boards[j].x, boards[j].y};
			 int br[2] = {boards[j].x + boards[j].width, boards[j].y + boards[j].height};
